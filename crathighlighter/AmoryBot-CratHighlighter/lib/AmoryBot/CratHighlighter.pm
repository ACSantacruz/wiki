...
use Carp;
# use Getopt::Long;		# FIXME TODO
use FindBin qw($Bin);
use List::Util qw(uniqstr);

use JSON::MaybeXS;
# use Log::Log4perl qw(:easy);	# FIXME TODO
use Log::Any qw($log);
use Git::Repository;
use MediaWiki::API;
...
use Exporter 'import';
our @EXPORT_OK = qw(gitCheck gitOnMain gitCleanStatus gitSHA mwLogin getConfig dieNice botShutoffs getCurrentGroups findLocalGroupMembers findArbComMembers getPageGroups processFileData cmpJSON changeSummary oxfordComma mapGroups usage);
...
sub gitCheck {
  ...
  if (gitCleanStatus($repo)) {
    croak $log->fatal('Repository is not clean');
  } elsif (gitOnMain($repo)) {
    croak $log->fatal('Not on main branch');
  }
  ...
  croak $log->fatal("Fetched and merged but SHAs are the same: $newSHA");
}
...
# Handle logging in to the wiki, mainly ensuring we croak nicely
sub mwLogin {
...
sub getConfig {
  ...
  my $config = shift;
  # Ensure full config
  croak $log->fatal('No username provided') if ! defined ${$config}{username};
  ...
  # chdir "$Bin" or croak $log->fatal('Failed to change directory');
  # chdir "$Bin" or croak 'Failed to change directory';
  chdir ${$config}{rcdir} or croak $log->fatal('Failed to change directory');

  my $correctname = ${$config}{username};
  my ($un, $pw);

  # FIXME TODO
  # open my $config, '<', '.crathighlighterrc' or croak $ERRNO;
  open my $rc, '<', '.crathighlighterrc' or croak $log->fatal($ERRNO);
  while (<$rc>) {
    chomp;
    ($un, $pw) = split /:/;
  ...
  close $rc or croak $log->fatal($ERRNO);
  # Only accept the right user
  if ($un !~ /^$correctname@/) {
    croak $log->fatal('Wrong user provided');
  }
  ${$config}{username} = $un;
  ${$config}{password} = $pw;
...
#     croak $log->fatal('DISABLED on-wiki');
#   }
...
#   if ($userNotes) {
#     croak $log->fatal("$bot has talkpage message(s))");
#   }
# }
...
#   # Should be enWiki specific? FIXME TODO
...
sub findLocalGroupMembers {
  ...
  foreach my $userHash (@{$localData}) {
    ...
    # Use map? FIXME TODO
    foreach my $group (grep {/$localRE/} @{${$userHash}{groups}}) {
...
# Maybe check if JSON() obj, decode otherwise?  Save doing above...
sub cmpJSON {
...
# Will need to restructure a bit, nbd FIXME TODO
...
Amory, C<< <Amorymeltzer at gmail.com> >>
...
This software is Copyright (c) 2021 by Amory.
